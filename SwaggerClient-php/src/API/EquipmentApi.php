...
class EquipmentApi
{
     ...
     * Add a new Equipment
     *
     * @param \Bumbal\Client\Model\EquipmentModel $body Equipment object that needs to be created (optional)
     ...
     * @throws \Bumbal\Client\ApiException on non-2xx response
     * @return \Bumbal\Client\Model\EquipmentModel
     */
     ...
     * @param int $equipment_id ID of equipment to retrieve (required)
     * @throws \Bumbal\Client\ApiException on non-2xx response
     * @return array of \Bumbal\Client\Model\EquipmentModel, HTTP status code, HTTP response headers (array of strings)
     */
    public function retrieveEquipmentWithHttpInfo($equipment_id)
    {
        ...
        try {
            list($response, $statusCode, $httpHeader) = $this->apiClient->callApi(
                ...
                $httpBody,
                $headerParams,
                '\Bumbal\Client\Model\EquipmentModel',
                '/equipment/{equipmentId}'
            );

            return [$this->apiClient->getSerializer()->deserialize($response, '\Bumbal\Client\Model\EquipmentModel', $httpHeader), $statusCode, $httpHeader];
        } catch (ApiException $e) {
            switch ($e->getCode()) {
                case 200:
                    $data = $this->apiClient->getSerializer()->deserialize($e->getResponseBody(), '\Bumbal\Client\Model\EquipmentModel', $e->getResponseHeaders());
                    $e->setResponseObject($data);
                    break;
            }
     ...
     *
     * @param \Bumbal\Client\Model\EquipmentRetrieveListArguments $arguments Equipment RetrieveList Arguments (required)
     * @throws \Bumbal\Client\ApiException on non-2xx response
     * @return \Bumbal\Client\Model\EquipmentModel[]
     */
